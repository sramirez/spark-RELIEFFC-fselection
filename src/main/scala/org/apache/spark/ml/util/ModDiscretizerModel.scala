 ...
 */
class ModDiscretizerModel(val thresholds: Array[Array[Float]]) extends VectorTransformer {
   ...
   */
  def transformRDD(data: RDD[LabeledPoint]) = {
    ...
    data.map { lp =>
      lp.features match {
        case v: SparseVector =>
          ...
          // the `index` array inside sparse vector object will not be changed,
          // so we can re-use it to save memory.
          LabeledPoint(lp.label, Vectors.sparse(v.size, v.indices, newValues.toArray))
          
        case v: DenseVector =>
          val newValues = for (i <- v.values.indices)
            yield assignDiscreteValue(v(i).toFloat, bc_thresholds.value(i)).toDouble         
          LabeledPoint(lp.label, Vectors.dense(newValues.toArray))
      }
    }
  }
