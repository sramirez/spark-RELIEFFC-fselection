 ...
 * @author sramirez
 */
class LongVectorAccumulator(val rows: Int) extends AccumulatorV2[Vector[Long], Vector[Long]] {
  ...
  def copy(): AccumulatorV2[Vector[Long], Vector[Long]] = new LongVectorAccumulator(accVector)
}